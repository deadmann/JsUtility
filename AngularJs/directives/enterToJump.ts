...
module AngularUtility {
    ...
    class TabIndexedContainer implements ng.IDirective {
        ...
        constructor() {
            ...
            this.link = function (scope: ng.IRepeatScope, element: IAugmentedJQuery, attrs: IEnterAttributes) {
    ...
    export interface IEnterToJumpAttribute extends ng.IAttributes {
        enterToJumpAttachToClosest: string;
        dataEnterToJumpAttachToClosest: string;
        enterAsTabAttachToClosest: string;
        dataEnterAsTabAttachToClosest: string;

        enterToJumpClosestElementTab: string;
        dataEnterToJumpClosestElementTab: string;
        enterAsTabClosestElementTab: string;
        dataEnterAsTabClosestElementTab: string;

        enterToJumpNextElementsConditions:string;
        dataEnterToJumpNextElementsConditions: string;
        enterAsTabNextElementsConditions:string;
        dataEnterAsTabNextElementsConditions:string;
    }
    ...
    class EnterToJumpDirective implements ng.IDirective {
        ...
        constructor() {
            ...
            this.link = function (scope: IRepeatScope, element: IAugmentedJQuery, attrs: IEnterToJumpAttribute, tabIndexContainerControllers: Array<ITabIndexedContainerController>) {
                ...
                var attachToClosestElem: IAugmentedJQuery;
                if (attachToClosest) {
                ...
                targetElement.bind("keydown keypress", function (event) {
                    if (event.which === 13) {
                        ...
                        if(nextJumpListElements.length == 0) {
                            ...
                            if(nextTabIndex == -1) { //If no match from priority list found
                                var nextTabIndex = Util.Utility.indexOf(enabledTabs, /**currentItem*/ tabIndexElement[0], function (arrItem, searchItem) {
                                        return arrItem.tabIndex === searchItem.tabIndex;
                                    }) + 1; //Never Can Be -1 (unless there is an error)
                            }
